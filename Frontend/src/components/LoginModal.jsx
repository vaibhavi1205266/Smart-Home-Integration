    ...
    <div className="fixed top-0 left-0 w-screen h-screen z-[9999] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white rounded-[40px] shadow-2xl overflow-hidden animate-slide-up">
        ...
        <div className="p-10">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <label className="block text-sm font-bold text-slate-700">Email or Mobile</label>
              <input
                type="text"
                name="contactInfo"
                value={formData.contactInfo}
                onChange={handleChange}
                className="w-full px-6 py-4 rounded-2xl bg-slate-50 border outline-none"
                required
              />
            </div>
            <div className="space-y-2">
            ...
            <button type="submit" disabled={isLoading} className="w-full py-4 bg-purple-600 text-white font-bold rounded-2xl">
              {isLoading ? "Signing in..." : "Log In"}
            </button>
          </form>
        </div>
